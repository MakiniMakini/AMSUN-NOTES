...
# ///////////////ALLOW USER TO EDIT THEIR NOTES\\\\\\\\\\\\\\\\\\\
@app.route("/<note_id>/edit", methods=["GET", "POST"])
@login_required
def edit(note_id):
    ...
    if request.method == "GET":
        ...
        db.execute("SELECT note_id, title, notes FROM notes WHERE user_id=:user_id AND note_id=:note_id", {'user_id':user_id, 'note_id':note_id})
        notes = db.fetchall()
        # populate text-area with existing notes
    ...
    if request.method == "POST":
        ...
        if not request.form.get("notes"):
            ...
            # update notes
            db.execute("UPDATE notes SET title =:title, notes =:notes WHERE note_id=:note_id", {'user_id':user_id, 'note_id':note_id, 'title':title, 'notes':notes})
            # save to data base
            connection.commit() 
            # redirect user to their notes
